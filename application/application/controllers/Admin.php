...
class Admin extends CI_Controller
{
    ...
    public function certificate($param='')
    {
        ...
        $page_data['lastCert']=$this->crud_model->getLastCert();
        if($param=='add'){
            
            if($page_data['lastCert'][0]->s_image==$this->input->post('path')){
                $dataArr=array(
                    'pk_i_id'=>$this->input->post('pk_i_id'),
                    's_image'=>$this->input->post('path'),
                    'x1'=>$this->input->post('x1'),
                    'y1'=>$this->input->post('y1'),
                    'x2'=>$this->input->post('x2'),
                    'y2'=>$this->input->post('y2'),
                );
                $this->crud_model->updateCert($dataArr);
            }
            else{
                $dataArr=array(
                    's_image'=>$this->input->post('path'),
                ...
                $this->crud_model->addCert($dataArr);
            }
    ...
    public function widget($param='',$id=0)
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        if($param=='add'){
            ...
            if($this->input->post('type')==2){
                if(isset($_FILES['content'])){
                    ...
                    if (!file_exists('uploads/wedget_files')) {
                        mkdir('uploads/wedget_files', 0777, true);
                    }
                    move_uploaded_file($_FILES['content']['tmp_name'], 'uploads/wedget_files/' . $uploadable_file. '.' . $fileExtension);
                    $fileName=$uploadable_file. '.' . $fileExtension;
                    $dataArr=array(
                    ...
                    $this->crud_model->addWedget($dataArr);
                }
            }
            else{
        ...
        if($param=='update'){
            ...
            if($this->input->post('type')==2){
                if(isset($_FILES['content'])){
                    ...
                    $this->crud_model->updateWedget($dataArr);
                }
            }
            else{
        ...
        $page_data['page_title'] = get_phrase('widget');
        $page_data['widget_list'] = $this->crud_model->getwidget();
        $this->load->view('backend/index.php', $page_data);
    }
    ...
    public function edit_widget($id=0)
    {
        ...
        $page_data['page_name'] = 'editWidget';
        $page_data['page_title'] = get_phrase('widget');
        $page_data['widget_list'] = $this->crud_model->getwidget($id);
        if(sizeof($page_data['widget_list'])==0)
        redirect('admin/widget', 'refresh');
    ...
    public function uploadCert(){
        ...
        if(isset($_FILES['cert_file'])){
            ...
            $fileExtension = end($tmp);
            $uploadable_file = md5(uniqid(rand(), true));
            if (!file_exists('uploads/cert_files')) {
                mkdir('uploads/cert_files', 0777, true);
            }
            move_uploaded_file($_FILES['cert_file']['tmp_name'], 'uploads/cert_files/' . $uploadable_file. '.' . $fileExtension);
            $fileName=$uploadable_file. '.' . $fileExtension;
        }
    ...
    public function about_us($param1 = "")
    {
        ...
        if ($param1 == "edit") {
            $this->crud_model->updateSettings("about_us");
            $this->session->set_flashdata('flash_message', get_phrase('data_updated_successfully'));
            redirect(site_url('admin/about_us'), 'refresh');
        }
    ...
    public function coupons($param1 = "", $id = null)
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }

        if ($param1 == "" || $param1 == "filter_by_date_range") {
            $page_data['coupons'] = $this->crud_model->couponGetAllByUserId();
            $page_data['timestamp_start'] = strtotime(date("m/d/Y"));
            $page_data['timestamp_end'] = strtotime('+365 days', time());
        }

        if ($param1 == "add") {
            $this->crud_model->couponAdd();
            $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            redirect(site_url('admin/coupons'), 'refresh');
        } elseif ($param1 == "edit") {
            $this->crud_model->couponUpdate($id);
            $this->session->set_flashdata('flash_message', get_phrase('data_updated_successfully'));
            redirect(site_url('admin/coupons'), 'refresh');
        } elseif ($param1 == "delete") {
            $this->crud_model->couponDelete($id);
            $this->session->set_flashdata('flash_message', get_phrase('data_deleted'));
            redirect(site_url('admin/coupons'), 'refresh');
        }
    ...
    public function coupon_form($param1 = null, $id = null)
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == "edit_coupon") {
            ...
            $page_data['coupon'] = $this->crud_model->couponFiendBy(array('key' => 'id', 'value' => $id));
        }

        $page_data['type'] = array('specific', 'public');
        //$page_data['type'] = array('public');
        //$page_data['courses'] = $this->crud_model->getAllCourses();
		$page_data['courses'] = $this->crud_model->getAllCoursesNotFreeNotDiscount();
        $page_data['timestamp_start'] = strtotime(date("m/d/Y"));
        $page_data['timestamp_end'] = strtotime('+365 days', time());
		
		//var_dump($page_data['courses']);
		//var_dump(date("m/d/Y",$page_data['timestamp_end']));exit;

        $this->load->view('backend/index', $page_data);
    }

    public function promotional_form($param1 = null)
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == "add_promotional") {
            $page_data['page_name'] = 'promotional_add';
            $page_data['page_title'] = get_phrase('add_promotional');
        }

        $page_data['promotionals'] = $this->crud_model->getAllPromotionalForUser();
        $page_data['courses'] = $this->crud_model->getAllCoursesForUser();
        $page_data['timestamp_start'] = strtotime(date("m/d/Y"));
        $page_data['timestamp_end'] = strtotime('+365 days', time());
    ...
    public function promotional($param1 = "", $id = null)
    {
        ...
        if ($param1 == "add") {
            $this->crud_model->promotionalAdd();
            $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            redirect(site_url('admin/promotional'), 'refresh');
        }

        $page_data['page_name'] = 'promotional_link';
        $page_data['page_title'] = get_phrase('promotional_link');
        $this->load->view('backend/index', $page_data);
    }

    public function couponDelete($id = null)
    {
        if ($this->session->userdata('admin_login') != true || is_null($id)) {
            redirect(site_url('login'), 'refresh');
        }
    ...
    public function conversions($action = null, $id = null , $param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($action != null && $id != null) {
			...
			//var_dump($date_range[0],$date_range[1]);
			...
			if(isset($_POST)){
				//var_dump($page_data['timestamp_start']);
			}//var_dump($where);exit;
        } else {
            $page_data['timestamp_start'] = strtotime('-29 days', time());
            $page_data['timestamp_end'] = strtotime(date("m/d/Y"));
        }
            $page_data['page_name'] = 'conversions_show';
            $page_data['page_title'] = get_phrase('user_conversions');
            $page_data['userid'] = $id; 
			$page_data['payment_history'] = $this->crud_model->getByLic($id,$where);
			//var_dump($id);exit;
			//echo $this->db->last_query();
			
			//$this->crud_model->get_instructor_revenue();
            //$page_data['accounts'] = $this->crud_model->getAllAccountsByUserId($id);
        }else{
            $page_data['page_name'] = 'conversions';
            $page_data['page_title'] = get_phrase('conversions');
			$page_data['payment_history'] = $this->crud_model->getAllAccountsByUserId(0);//$this->crud_model->get_instructor_revenue();
			//var_dump($page_data['payment_history']);exit;
			//var_dump($page_data['total2']);exit;

            //$page_data['accounts'] = $this->crud_model->getAllAccounts();
        }
    ...
    public function exportMasPayment($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        
		$res = $this->crud_model->getAllAccountsByUserId(0);
		
        $filename = $param1."_" . date('Ymd') . ".csv";
        header("Content-Disposition: attachment; filename=\"$filename\"");
        header("Content-Type: application/vnd.ms-excel");
        /*echo"<table>";
        foreach($res as $row){
            if($param1==$row->payment_type)
                if(((double)$row->sumIPaid-(double)$row->sumPaid)>0)
                    echo "<tr>
                        <td>
                            ".$row->payment_id."
                        </td>
                        <td>
                            ...
                            ".((double)$row->sumIPaid-(double)$row->sumPaid)."
                        </td>
                        <td>
                            USD
                        </td>
        ...
        echo "</table>";*/
        $f = fopen(getcwd()."/uploads/".$filename."", "w");
        foreach($res as $row){
            if($param1==$row->payment_type)
                if(((double)$row->sumIPaid-(double)$row->sumPaid)>0){
                    $rowArr=array(
                        'userId'=>$row->payment_id,
                        'amount'=>(double)$row->sumIPaid-(double)$row->sumPaid,
                        'Currency'=>'USD',
                    );
                    fputcsv($f, $rowArr);
                }
        }
        redirect("/uploads/".$filename."","redirect");
    }
    ...
    public function users($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {
            ...
            $this->user_model->ban_user($param2);
            redirect(site_url('admin/users'), 'refresh');
        }elseif ($param1 == "unban") {
            $this->user_model->unban_user($param2);
            redirect(site_url('admin/users'), 'refresh');
        }
    ...
    public function user_form($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'add_user_form') {
            ...
            $page_data['page_title'] = get_phrase('view_student_data');
            $this->load->view('backend/index', $page_data);
        }
    }
    ...
    public function get_courses()
    {
        ...
        <div class="dropright dropright">
          ...
          <ul class="dropdown-menu">
              ...
              <li><a class="dropdown-item" href="' . $edit_this_course_url . '">' . get_phrase("show_this_course") . '</a></li>
    ...
    public function course_actions($param1 = "", $param2 = "")
    {
        ...
        } /*elseif ($param1 == "active_course") {
            $this->crud_model->active_course($param2);
            redirect(site_url('admin/courses'), 'refresh');

        }elseif ($param1 == "pending_course") {
            $this->crud_model->pending_course($param2);
            redirect(site_url('admin/courses'), 'refresh');
    ...
    public function stripe_checkout_for_instructor_revenue()
    {
        ...
        $page_data['amount_to_pay'] = $this->input->post('amount_to_pay');
			//var_dump($page_data['amount_to_pay']);exit;
        $page_data['payment_id'] = $this->input->post('payment_id');
        $page_data['course_title'] = $this->input->post('course_title');
        $page_data['instructor_name'] = $this->input->post('instructor_name');
    ...
    public function contact_us()
    {
        ...
        // $page_data['contact_us']=  $this->crud_model->getUnreadMsg();
        $page_data['contact_us'] = $this->crud_model->getAllContacts();
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function deletecontact($id)
    {
        $page_data['page_name'] = 'contact';
        // $page_data['contact_us']=  $this->crud_model->getUnreadMsg();
        $this->crud_model->deleteMsgContact($id);
        $this->load->view('backend/index', $page_data);
    ...
    {
        $page_data['page_name'] = 'unread_contact';
        $page_data['contact_us'] = $this->crud_model->getUnreadMsg();
        $this->load->view('backend/index', $page_data);
    }
    ...
    {
        $page_data['page_name'] = 'showcontact';
        $page_data['showContact'] = $this->crud_model->getContact($id);
        $this->load->view('backend/index', $page_data);
        $this->crud_model->updateMsgRead($id);
    }
