...
class User extends CI_Controller
{
    ...
    public function course_actions($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {
			/*if($_POST){
				list($width, $height, $type, $attr) =getimagesize($_FILES['course_thumbnail']["tmp_name"]);
				//var_dump($width, );exit;
				if ($_FILES['course_thumbnail']['name'] != "") {
					...
					{
						$this->session->set_flashdata('errorImg',  'Error: Your image size less than 600px*600px');
						redirect(site_url('user/course_form/add_course')	, 'refresh');
					}else
						$this->session->set_flashdata('errorImg',  '');


					
				}
			}*/
            $course_id=$this->crud_model->add_course($param2);
            ...
            redirect(site_url('user/cou+++rses'), 'refresh');
        } elseif ($param1 == 'draft') {
            $this->is_the_course_belongs_to_current_instructor($param2);
    ...
    public function coupons($param1 = "", $id = null)
    {
        ...
        if ($param1 == "add") {
            ...
            redirect(site_url('user/coupons'), 'refresh');
        } elseif ($param1 == "edit") {
    ...
    public function coupon_form($param1 = null, $id = null)
    {
        ...
        if ($param1 == "show_coupon") {
            $page_data['page_name'] = 'coupon_show';
            $page_data['page_title'] = get_phrase('show_coupon');
            $page_data['coupon'] = $this->crud_model->couponFiendBy(array('key' => 'id', 'value' => $id));
        }

//        $page_data['type'] = array('specific', 'public');
        $page_data['type'] = array('specific');
        //$page_data['courses'] = $this->crud_model->getAllCoursesForUser();
        $page_data['courses'] = $this->crud_model->getAllCoursesForUserNotFreeNotDiscount();
        $page_data['timestamp_start'] = strtotime(date("m/d/Y"));
        $page_data['timestamp_end'] = strtotime('+365 days', time());
    ...
    public function promotional($param1 = "", $id = null)
    {
        ...
        if ($param1 == "add") {
            ...
            $this->session->set_flashdata('flash_message', get_phrase('data_added_successfully'));
            redirect(site_url('user/promotional'), 'refresh');
        }
    ...
    {
        if ($this->session->userdata('user_login') != true || is_null($id)) {
            redirect(site_url('login'), 'refresh');
        }
    ...
    public function instructor_revenue($param1 = "")
    {
		...
		//var_dump(sizeof($page_data['payment_history']));//exit;
    ...
    function save_course_progress()
    {
        ...
        if($response==100){
            $resLesson=$this->db->get_where('lesson', array('id' => $this->input->post('lesson_id')))->result();
            if(sizeof($resLesson)>0){
                $resCert=$this->crud_model->getCertUserCourse($this->session->userdata('user_id'),$resLesson[0]->course_id);
            /*echo $this->db->last_query();exit;
                
                    var_dump($resCert);*/
                if(sizeof($resCert)==0){
                    $lastCert=$this->crud_model->getLastCert();
                    $dataArr=array(
                        'fk_i_cert_id'=>$lastCert[0]->pk_i_id,
                        'fk_i_user_id'=>$this->session->userdata('user_id'),
                        'fk_i_crs_id'=>$resLesson[0]->course_id,
                        'dt_created_at'=>date('Y-m-d'),
                        'serial_no'=>$this->session->userdata('user_id').date('Ymd').$resLesson[0]->course_id,
                    );
                   $this->crud_model->addUserCert($dataArr);
                   //echo $this->db->last_query();
                }
    ...
    public function DoUpload(){
        $img='';
        if(isset($_FILES['course_thumbnail']) && strlen($_FILES['course_thumbnail']['name'])>0){
            $imgParts=explode('.',$_FILES['course_thumbnail']['name']);
            $ext=$imgParts[sizeof($imgParts)-1];
            //var_dump($imgParts);
            if(in_array(strtolower($ext),array('png','gif','jpg'))){
                $img=time().rand(1000,9999).'.'.$ext;
                move_uploaded_file($_FILES['course_thumbnail']['tmp_name'],getcwd().'/uploads/'.$img);
            }
        }
        echo json_encode( $img);
    }
    
    public function DoimgCropper(){
        ...
        $w=$this->input->post('w');
        $h=$this->input->post('h');
        $arr_image_details = getimagesize(getcwd().'/uploads/'.$img);
        //var_dump();exit;
        if ($arr_image_details['mime'] == 'image/gif') {
            $im = ImageCreateFromGIF(getcwd().'/uploads/'.$img);
            $im2 = imagecrop($im, array('x' => $x1, 'y' => $y1, 'width' => $w, 'height' => $h));
            ImageGIF($im2, getcwd().'/uploads/'.'t'.$img);
        }
        if ($arr_image_details['mime'] == 'image/jpeg' ||  $arr_image_details['mime']=='image/jpg') {
            $im = ImageCreateFromJPEG(getcwd().'/uploads/'.$img);
            $im2 = imagecrop($im, array('x' => $x1, 'y' => $y1, 'width' => $w, 'height' => $h));
            ImageJPEG($im2, getcwd().'/uploads/'.'t'.$img);
        }
        if ($arr_image_details['mime'] == 'image/png') {
            $im = ImageCreateFromPNG(getcwd().'/uploads/'.$img);
            $im2 = imagecrop($im,  array('x' => $x1, 'y' => $y1, 'width' => $w, 'height' => $h));
            imagepng($im2, getcwd().'/uploads/'.'t'.$img);
        }
        echo 1;
    }
    public function imgCropper($file=''){
        $this->data['img']=$file;
        $this->load->view('backend/user/cropImage',$this->data);
    }
}
