...
class Home extends CI_Controller
{
    ...
    public function courses()
    {
        ...
        $page_data['widget'] = $this->crud_model->getwidget($dataArr);
        ...
        // Get the crs
        if (isset($_GET['crs']) && !empty($_GET['crs'] && $_GET['category'] != "crs")) {
            $selected_crs = urlencode($_GET['crs']);
        }
        ...
        $page_data['selected_crs'] = $selected_crs;
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function course($slug = "", $course_id = "")
    {
        ...
        $page_data['page_title'] = get_phrase('course');
            ...
            $query = $this->db->get();
            $page_data['CourseData']=$query->result();
            //var_dump($page_data['CourseData']);
        if ($this->session->userdata('user_id') != Null) {
            ...
            if (count($query->result_array()) == 0) {
                ...
                $this->db->insert('recent_courses', $data);
            }
            if (count($query->result_array()) > 0) {
                $data = array(
                    'created_at' => date('Y-m-d H:i:s'),
                ...
                $this->db->where('user_id', $this->session->userdata('user_id'));
                $this->db->where('course_id', $course_id);
                $this->db->update('recent_courses', $data);
            }
        }
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function profile($param1 = "")
    {
        ...
        } elseif ($param1 == 'user_payment') {
            $page_data['page_name'] = "update_user_payment";
            $page_data['page_title'] = get_phrase('update_user_payment');
        }
    ...
    public function update_profile($param1 = "")
    {
        ...
        } elseif ($param1 == "update_payment") {
            $this->user_model->update_payment($this->session->userdata('user_id'));
            $this->session->set_flashdata('flash_message', get_phrase('updated_successfully'));
        }
    ...
    public function handleWishList()
    {
        ...
        if (!$this->session->userdata('wish_items')) {
            $this->session->set_userdata('wish_items', array());
        }
        if ($this->session->userdata('user_login') != 1) {
            ...
            if (isset($_POST['course_id'])) {
                ...
                $wishlists=$this->crud_model->handleWishList($course_id,$state);//$user_details['wishlist']
            }
    ...
    public function handleWishListItems()
    {
        if (!$this->session->userdata('wish_items')) {
            $this->session->set_userdata('wish_items', array());
        }
        ...
        $course_id = $this->input->post('course_id');
        $previous_cart_items = $this->session->userdata('wish_items');
        if (in_array($course_id, $previous_cart_items)) {
            $key = array_search($course_id, $previous_cart_items);
            unset($previous_cart_items[$key]);
        } else {
        ...
        $this->session->set_userdata('wish_items', $previous_cart_items);
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/wishlist_items');
    }
    ...
    public function contact_us()
    {
        if(isset($_POST)){
            ...
            if(isset($user)){
				...
				$data['name']=$role_id==3?'Customer: '.$this->session->userdata('name'):'Instructor: '.$this->session->userdata('name');
            }else
            ...
            $data['updated_at'] = date('Y-m-d h:s:i', time());
    ...
    //var_dump($data);exit;
    		...
    		if($this->form_validation->run() !== false){
                $this->crud_model->addContact($data);
                $this->email_model->send_smtp_mail('Name: '.$data['name'].'<br />Email: '.$data['email'].'<br/>Message: '.$data['msg'], 'Contact us Form', get_settings('system_email'), 'info@'.get_settings('smtp_host'));
            }
        }
        //
    ...
    public function login()
    {
        ...
        $page_data['loginURL'] = $this->googleplus->loginURL();
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function add_coupon($code = null, $courseId)
    {
		if($this->crud_model->addCouponSession($code, $courseId)){
	...
	user_certificate.pk_i_id,
    user_certificate.dt_created_at,
    user_certificate.serial_no,
    users.first_name,
    users.last_name,
    course.title,
    certificate.s_image,
    ...
    certificate.x2,
    certificate.y2
FROM `user_certificate` join users
on user_certificate.fk_i_user_id=users.id
join course
on user_certificate.fk_i_crs_id=course.id
join certificate
on user_certificate.fk_i_cert_id=certificate.pk_i_id where serial_no='".$id."'");
$certArr=$q->result();

$data['lastCert']=$certArr;
if(sizeof($certArr)>0)
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/certificate', $data);
        else
        redirect('/','refresh');
