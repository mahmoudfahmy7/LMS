...
<form action="<?php echo site_url('user/lessons/'.$param3.'/edit'.'/'.$param2); ?>" method="post" enctype="multipart/form-data">
    ...
    <div class="" id="video" <?php if($lesson_details['lesson_type'] != 'video'):?> style="display: none;" <?php endif; ?>>
        ...
        <div class="" id = "html5" <?php if($lesson_details['video_type'] != 'html5'): ?> style="display: none;" <?php endif; ?>>
            <div class="form-group" style="display:none">
                <label><?php echo get_phrase('video_url'); ?></label>
                <input type="text" id = "html5_video_urlwww" name = "html5_video_urlwww" class="form-control" value="<?php echo $lesson_details['video_url']; ?>">
            </div>

            <div class="form-group">
                <label><?php echo get_phrase('video_url'); ?> mp4</label>
                <input type="file" id = "html5_video_url" name = "html5_video_url">
                <label><?php echo get_phrase('srt1'); ?></label>
                <input type="file" id = "html5_video_srt" name = "html5_video_srt[]" multiple><span style="color:#ff0000"><?php echo get_phrase('srt1x'); ?></span>
            </div>
	...
	<div class="form-group">
		<div class="custom-control custom-checkbox">
			<input type="checkbox" class="custom-control-input" name="preview" id="preview" <?php if($lesson_details['preview'] == 1) echo 'checked'; ?> >
			<label class="custom-control-label" for="preview"><?php echo get_phrase('check_if_this_is_a_preview_lesson'); ?></label>
		</div>
	</div>
...
document.getElementById('html5_video_url').onchange = setFileInfo;
...
function updateInfos() {
  /*var infos = document.getElementById('html5_duration');
  infos.textContent = "";*/
  for (var i = 0; i < myVideos.length; i++) {
    $("#html5_duration").val(toHHMMSS(myVideos[i].duration));
  }
}
