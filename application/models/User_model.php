...
//        $this->db->where('role_id', 2);
    ...
    public function update_payment($user_id = "") { // Admin does this editing

            $data['payment_type'] = html_escape($this->input->post('payment_type'));
            $data['payment_id'] = html_escape($this->input->post('payment_id'));
            $data['last_modified'] = strtotime(date("Y-m-d H:i:s"));
	...
	public function ban_user($user_id = "") {
        ...
        $this->db->where('id', $user_id);
        $this->db->update('users',$data);
        $this->session->set_flashdata('flash_message', get_phrase('user_ban_successfully'));
    }
	public function unban_user($user_id = "") {
		$data['status']=1;
        $this->db->where('id', $user_id);
        $this->db->update('users',$data);
        $this->session->set_flashdata('flash_message', get_phrase('user_unban_successfully'));
    ...
    public function upload_user_image($user_id) {
        if (isset($_FILES['user_image']) && $_FILES['user_image']['name'] != "") {
           // echo getcwd().'uploads/user_image/'.$user_id.'.jpg';
            if(file_exists(getcwd().'/uploads/user_image/'.$user_id.'.jpg')){
                unlink(getcwd().'/uploads/user_image/'.$user_id.'.jpg');
            }
            move_uploaded_file($_FILES['user_image']['tmp_name'], 'uploads/user_image/'.$user_id.'.jpg');
            chmod(getcwd().'/uploads/user_image/'.$user_id.'.jpg', 0755);
            $this->session->set_flashdata('flash_message', get_phrase('user_update_successfully'));
        }
    }
